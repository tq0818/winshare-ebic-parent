(function ($) {
    $.fn.treeTr = function (opts) {
        opts = $.extend({
            theme: 'default',
            expandLevel: 1,
            column: 0, //可控制列的序号,默认0,即第一列
            onSelect: function($treeTr, id){},
            beforeExpand: function($treeTr, id){}
        }, opts);

        var $treeTr = this;
        $treeTr.addClass('tree_table');
        ...
        var pMap = {}, cMap = {};
        var $trs = $treeTr.find('tr');
        initRelation($trs, true);    

        $treeTr.click(function (event) {
            ...
            if ($target.attr('controller')) {
                ...
                if (opts.onSelect && opts.onSelect($treeTr, id) === false) { return; }
            }
		...
		$treeTr.mouseover(hoverActiveNode).mouseout(hoverActiveNode);
        ...
        function open(id) {
            ...
            for (var i = 0; i < pMap[id].length; i++) {
                var cId = pMap[id][i];
                if (pMap[cId]) {
                    var className = $('#' + cId, $treeTr).find('.' + css['AN']).attr('class');
        ...
        function formatNode(tr) {
            ...
            if (cMap[id] == 0) {
                ...
                if (!$cur.attr('isFirstOne')) {
                    var $preSpan = $('#' + $cur.attr('prevId'), $treeTr).children("td").eq(opts.column).find('.prev_sp').clone();
                } else {
                    var $parent = $('#' + cMap[id], $treeTr);
        ...
        $treeTr.addChilds = function(trsHtml) {
            var $trs = $(trsHtml);
